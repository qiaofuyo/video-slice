...
const selectedFiles = []; // 存储已选择的 File 对象列表
...
const fastBackwardSecondsInput = document.getElementById(
  'fast-backward-seconds'
);
const longFastForwardMinutesInput = document.getElementById(
  'long-fast-forward-minutes'
);
const longFastBackwardMinutesInput = document.getElementById(
  'long-fast-backward-minutes'
);
...
function parseTime(timeString) {
  ...
  const h = Number.parseInt(padded.slice(-6, -4), 10);
  const m = Number.parseInt(padded.slice(-4, -2), 10);
  const sec = Number.parseInt(padded.slice(-2), 10);
...
function renderFileList() {
  ...
  selectedFiles.forEach((file, idx) => {
    ...
    div.className =
      'flex items-center justify-between p-2 bg-gray-100 rounded-lg border border-gray-200';
...
function centerVideoWindow() {
  ...
  const ew = videoFloatWindow.offsetWidth,
    eh = videoFloatWindow.offsetHeight;
...
markStartBtn.addEventListener('click', () => {
  if (!(currentFile || videoPlayer.src)) {
    showMessage('请先选择并播放视频');
    return;
  }
...
addClipBtn.addEventListener('click', () => {
  ...
  try {
    const base = basenameNoExt(currentFile.name);
    outputHost = base.split('_')[0] || base;
    outputHost = outputHost.replace(/[/\\:*?"<>|]/g, '_');
  } catch (e) {
    outputHost = 'unknown';
  }
  ...
  try {
    if (currentFile && currentFile.lastModified)
      fileDate = formatDateYYYYMMDD(currentFile.lastModified);
    else fileDate = formatDateYYYYMMDD(Date.now());
  } catch (e) {
    fileDate = formatDateYYYYMMDD(Date.now());
  }
  ...
  renderClipsList();
  showMessage(
    `已添加片段：${currentFile.name} (${formatTime(startSec)} - ${formatTime(endSec)})`
  );
});
...
function getNextIndexForHostDate(outputHost, fileDate) {
  ...
  if (Array.isArray(clips)) {
    for (let i = 0; i < clips.length; i++) {
      const c = clips[i];
      if (c && c.outputHost === outputHost && c.fileDate === fileDate)
        sameCount++;
    }
  }
  return ++sameCount;
...
fileListDiv.addEventListener('click', (e) => {
  ...
  if (playBtn) {
    const idx = Number.parseInt(playBtn.dataset.index, 10);
    const file = selectedFiles[idx];
    if (file) playSelectedFile(file);
    return;
  }
  if (delBtn) {
    const idx = Number.parseInt(delBtn.dataset.index, 10);
...
addedClipsList.addEventListener('click', (e) => {
  ...
  if (playBtn) {
    ...
    const originalFile = selectedFiles.find(
      (f) => f.name === clip.fileName && f.size === clip.originalFileSize
    );
    if (!originalFile) {
      showMessage(`错误：找不到原始文件 ${clip.fileName}`);
      return;
    }
  ...
  if (delBtn) {
    ...
    const removed = clips.splice(idx, 1)[0];
    // 更新 clips
    for (let index = idx; index < clips.length; index++) {
      const clip = clips[index];
      if (
        removed.outputHost === clip.outputHost &&
        removed.fileDate === clip.fileDate
      ) {
        clip.idx--;
...
addedClipsList.addEventListener('input', (e) => {
  ...
  if (nameInp) {
    const idx = Number.parseInt(nameInp.dataset.index, 10);
  ...
  if (extInp) {
    const idx = Number.parseInt(extInp.dataset.index, 10);
    if (!isNaN(idx) && clips[idx]) {
...
function playClipRange(clip) {
  ...
  videoPlayer.play().catch(() => {});
  showMessage(
    `正在播放片段: ${clip.fileName} (${clip.startTime} - ${clip.endTime})`
  );
}
...
generateBtn.addEventListener('click', () => {
  ...
  if (!srcDir) {
    outputCommands.value = '请先输入视频文件所在目录。';
    showMessage('请输入视频文件目录');
    return;
  }
  if (!outDir) {
    outputCommands.value = '请先输入输出目录。';
    showMessage('请输入输出目录');
    return;
  }
...
copyBtn.addEventListener('click', () => {
  if (!outputCommands.value) {
    showMessage('没有可复制的指令');
    return;
  }
...
function seekVideo(seconds) {
  ...
  videoPlayer.currentTime = Math.max(
    0,
    Math.min(newTime, videoPlayer.duration)
  );
  const direction = seconds > 0 ? '前进' : '后退';
  const unit = Math.abs(seconds) >= 60 ? '分钟' : '秒';
  const amount =
    Math.abs(seconds) >= 60 ? Math.abs(seconds) / 60 : Math.abs(seconds);
  showMessage(`${direction} ${amount} ${unit}`);
}

forwardBtn.addEventListener('click', () => {
  const seconds = Number.parseFloat(fastForwardSecondsInput.value) || 15;
  seekVideo(seconds);
});

rewindBtn.addEventListener('click', () => {
  const seconds = Number.parseFloat(fastBackwardSecondsInput.value) || 5;
  seekVideo(-seconds);
});

longForwardBtn.addEventListener('click', () => {
  const minutes = Number.parseFloat(longFastForwardMinutesInput.value) || 2;
  seekVideo(minutes * 60);
});

longRewindBtn.addEventListener('click', () => {
  const minutes = Number.parseFloat(longFastBackwardMinutesInput.value) || 2;
  seekVideo(-minutes * 60);
});
...
document.addEventListener('keydown', (e) => {
  ...
  const ffSec = Number.parseFloat(fastForwardSecondsInput.value) || 15;
  const fbSec = Number.parseFloat(fastBackwardSecondsInput.value) || 5;
  const longFfMin = Number.parseFloat(longFastForwardMinutesInput.value) || 2;
  const longFbMin = Number.parseFloat(longFastBackwardMinutesInput.value) || 2;

  if (e.ctrlKey) {
    if (e.key === 'ArrowRight') {
      e.preventDefault();
      seekVideo(longFfMin * 60);
    }
    if (e.key === 'ArrowLeft') {
      e.preventDefault();
      seekVideo(-longFbMin * 60);
    }
  } else {
    if (e.key === 'ArrowRight') {
      e.preventDefault();
...
resizeHandle.addEventListener('mousedown', (e) => {
  ...
  const aspect =
    videoPlayer && videoPlayer.videoWidth && videoPlayer.videoHeight
